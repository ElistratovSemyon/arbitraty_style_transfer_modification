...
class Ghiasi(nn.Module):
    def __init__(self, identity_connections=False):
        ...
        self.identity_connections = identity_connections

    def forward(self,x,styles, alpha):
        # x: B x 3 x H x W
        # styles: B x 100 batch of style embeddings
        if self.identity_connections:
            conv_res = []
            ind = 2
        for i, layer in enumerate(self.layers):
            if i < 3:
                ...
                if self.identity_connections:
                    #print("New  ", layer(x, styles).shape)
                    #print("OLD  ", conv_res[ind].shape)
                    x = alpha * layer(x, styles) + (1-alpha) * conv_res[ind]
                    ind -= 1
                else:
                    x = layer(x, styles)
            else:
