...
class StylePredictor(nn.Module):
    def __init__(self, pretrained=True, dropout=None):
        ...
        if pretrained:
            #inc = torch.hub.load('pytorch/vision:v0.6.0', 'inception_v3', pretrained=True)
            inc = torchvision.models.inception_v3(pretrained=True)
            self.load_state_dict(inc.state_dict(), strict=False)
    ...
    def forward(self, x):
        ...
        # mean-pool the convolutional features (global average):
